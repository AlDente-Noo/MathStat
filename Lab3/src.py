...
if __name__ == "__main__":
    data1 = load_csv('data/channel_1.csv')
    data2 = load_csv('data/channel_2.csv')
    data1_A, data1_B, data1_w = load_octave('data/Ch_1.txt')
    data2_A, data2_B, data2_w = load_octave('data/Ch_2.txt')
    data_n = [t for t in range(1, len(data1) + 1)]
    # plot data together
    plt.plot(data_n, [data1[i][0] for i in range(len(data1))], label='Ch1_500nm_0.23.csv')
    plt.plot(data_n, [data2[i][0] for i in range(len(data2))], label='Ch2_500nm_0.23.csv')
    plt.legend()
